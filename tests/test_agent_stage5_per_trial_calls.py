...
check("1i  ...it is a RuntimeError subclass, deliberately not a ValueError, "
      ...
      (True, False))
check("1i  ...non-degeneracy: on a SUPPORTED provider it does not fire, so "
      "the rows above are a measurement rather than a guard that always "
      "raises. This said 'the shipped provider' and set OPENAI to get it; "
      "since the provider flip the shipped provider is Converse -- which this "
      "gate also admits, and which this file does not run -- so the label now "
      "says what the assignment two lines above actually does",
      _prov_ok, None)
check("1i  ...and the provider was restored to what this file pins, which is "
      "the OpenAI arm rather than the shipped default",
      (config.MATCHING_PROVIDER, _saved_provider),
      (config.MATCHING_PROVIDER_OPENAI, config.MATCHING_PROVIDER_OPENAI))
