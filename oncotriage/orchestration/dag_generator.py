...
That is also why the generated text still says "03- Config.py" in two places --
the module docstring's Schedule line and the comment above DAG_SCHEDULE. Both
are PROSE inside the generated string, both are stale (item 20c retargeted the
two readers to ``oncotriage/config.py``, which is what the code below them
actually opens), and correcting either one would have changed the bytes and made
the equivalence proof for this pass meaningless. Fixing the prose is a one-line
follow-up whose acceptance criterion is a NEW sha256, deliberately not bundled
with a relocation.
...
THE DAG DUPLICATES THE SCRAPE AND THE INDEX BUILD, and that is a real cost
--------------------------------------------------------------------------
``_split_inclusion_exclusion``, ``_parse_trial_metadata`` and
``_create_trial_embedding_text`` inside the generated text are copies of logic
that also lives in ``oncotriage/retrieval/indexer.py``, and they can drift.
They are copies because the scheduler parses the DAG in its own process, which
has no ``oncotriage`` on its path and no reason to. Making the DAG import the
package instead is a real option and a real change -- it needs the package
installed into the Airflow environment -- and it is NOT this pass. Recorded
here so the duplication is a known cost rather than a discovery.
"""
